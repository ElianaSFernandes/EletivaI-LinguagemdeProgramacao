<?php
    ...
    function retornaDadosUsuario(){ // RETORNA OS DADOS DO USUÁRIO LOGADO 
        require("conexao.php");
    ...
    function alterarDadosUsuario ($nome, $email){ // PERMITE ALTERAR DADOS DO USUÁRIO LOGADO
        require ("conexao.php");
        try {
            $sql = "UPDATE usuario SET nome = ?, email = ? WHERE idusuario = ?";// EFETUA AS ALTERAÇÕE NO BANCO DE DADOS
            $stmt = $pdo -> prepare ($sql);
    ...
    function alterarSenha ($senhaAntiga, $novaSenha, $novaSenhaConfirm){ // EFETUA A TROCA DE SENHA
        require ("conexao.php");
        try{
            if ($novaSenha == $novaSenhaConfirm){
                $usuario = retornaDadosUsuario();

                if (password_verify ($senhaAntiga, $usuario ['senha'])){ // VERIFICA A SENHA ATUAL ANTES DE EFETUAR A ALTERAÇÃO
                ...
                echo "< class = 'text-danger'> Senhas não conferem! </p>";
            }
...
?>
    ...
    <h3> Alteração da senha </h3> <!-- FORMULÁRIO PARA ALTERAÇÃO DE SENHA -->
